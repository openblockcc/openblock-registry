 ...
 */
async function fetchPackagesJson() {
 ...
 */
function generateTableRow(toolchain) {
    const availableHosts = new Set(toolchain.systems.map(s => s.host));
    const cells = PLATFORMS.map(platform => availableHosts.has(platform) ? '✓' : '');
 ...
 */
function generateMarkdown(toolchains) {
